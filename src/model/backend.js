...
class Backend{
    ...
    async getDocFromImage(image) {
        try {
            ...
            let result = String(response.data['body']).split(/\\n/)
            for (var i = 0; i < result.length; i++){
                if (result[i] === "Product Name:")
                    temp['productName'] = result[i + 1]
                if (result[i] === "Supplier:")
                    temp['supplier'] = result[i + 1]
                if (result[i] === "EPA Registration #")
                    temp['epaRegNum'] = result[i + 1]
                if (result[i] === "EPA Est. #")
                    temp['epaEstNum'] = result[i + 1]
                if (result[i] === "Target:")
                    temp['target'] = result[i + 1]
                if (result[i] === "Vehicle:")
                    temp['vehicle'] = result[i + 1]
                if (result[i] === "Gear:")
                    temp['gear'] = result[i + 1]
                if (result[i] === "RPM:")
                    temp['rpm'] = result[i + 1]
                if (result[i] === "MPH:")
                    temp['mph'] = result[i + 1]
                if (result[i] === "Sprayer/Spreader:")
                    temp['sprayer'] = result[i + 1]
                if (result[i] === "Nozzles/Setting:")
                    temp['nozzle'] = result[i + 1]
                if (result[i] === "Pressure:")
                    temp['pressure'] = result[i + 1]
                if (result[i] === "Tank Mix: Amount of Product:")
                    temp['tankAmt'] = result[i + 1]
                if (result[i] === "`per")
                    temp['tankWater'] = result[i + 1]
                if (result[i] === "Adjuvant/Dye:")
                    temp['adjuvant'] = result[i + 1]
                if (result[i] === "Total Amount of Product Applied:")
                    temp['totalApplied'] = result[i + 1]
                if (result[i] === "Application rate:")
                    temp['appRateOz'] = result[i + 1]
                if (result[i] === "Minutes:")
                    temp['wateredMin'] = result[i + 1]
                if (result[i] === "Temperature:")
                    temp['temp'] = result[i + 1]
                if (result[i] === "F Humidity:")
                    temp['humidity'] = result[i + 1]
                if (result[i] === "% Wind:")
                    temp['wind'] = result[i + 1]
                if (result[i] === "Date of Application:")
                    temp['date'] = result[i + 1]
                if (result[i] === "PURS")
                    temp['purs'] = result[i + 1]
                if (result[i] === "Time of Application:")
                    temp['timeStart'] = result[i + 1]
                if (result[i] === "to")
                    temp['timeEnd'] = result[i + 1]
                if (result[i] === "How was container disposed:")
                    temp['disposed'] = result[i + 1]
                if (result[i] === "How was equipment cleaned:")
                    temp['cleaned'] = result[i + 1]
                ...
                if (result[i] === "Actual lbs of K2 0 applied per 1000 ft. 2")
                    temp['lbsK2O'] = result[i - 1]
                if (result[i] === "Signature:")
                    temp['signature'] = result[i + 1]
                if (result[i] === "Date:")
                    temp['sigDate'] = result[i + 1]
            }
            return Document(temp);
        } catch (error) {
            console.log("Error")
    ...
    async delete(id) {
        ...
        return response;
>>>>>>> 117953654a31dc04b39121f35f453a8c37d8b324
    }
  }
  convertBase64 = (image) => {
    return new Promise((resolve, reject) => {
